...
serve(async (req) => {
  ...
  try {
    ...
    if (!customerId) {
      ...
      logStep("Found customer via email search", { customerId });
    } else {
      logStep("Found customer via profile", { customerId });
    }

    const origin = req.headers.get("origin") || "https://swinginsitute.lovable.app";
